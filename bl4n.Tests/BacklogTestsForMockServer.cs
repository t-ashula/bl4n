...
namespace BL4N.Tests
{
    ...
    public class BacklogTestsForMockServer : BacklogTests, IClassFixture<BacklogMockServerFixture>
    {
        ...
        public override void DeleteProjectUserTest()
        {
            ...
            var backlog = new Backlog(Settings);
            var uid = new Random().Next();
            var actual = backlog.DeleteProjectUser(1, uid);
            Assert.Equal(uid, actual.Id);
            Assert.Equal("admin", actual.UserId);
            Assert.Equal("admin", actual.Name);
            Assert.Equal(1, actual.RoleType);
        ...
        public override void AddProjectAdministratorTest()
        {
            ...
            var backlog = new Backlog(Settings);
            var uid = new Random().Next();
            var actual = backlog.AddProjectAdministrator(1, uid);
            Assert.Equal(uid, actual.Id);
        ...
        public override void GetProjectAdministratorsTest()
        {
            ...
            var backlog = new Backlog(Settings);
            var actual = backlog.GetProjectAdministrators(1);
            Assert.Equal(1, actual.Count);
        ...
        public override void DeleteProjectAdministratorTest()
        {
            ...
            var backlog = new Backlog(Settings);
            var uid = new Random().Next();
            var actual = backlog.DeleteProjectAdministrator(1, uid);
            Assert.Equal(uid, actual.Id);
            Assert.Equal("takada", actual.UserId);
            Assert.Equal("takada", actual.Name);
            Assert.Equal(2, actual.RoleType);
        ...
        public override void DeleteProjectIssueTypeTest()
        {
            ...
            var actual = backlog.DeleteProjectIssueType(1, issueId, 2);
        ...
        public override void GetProjectGitRepositoryPullRequestsTest()
        {
            ...
            var projectKey = string.Format("p{0:X}", new Random().Next(10000));
            var repoName = string.Format("r{0:X}", new Random().Next(100));
            var actual = backlog.GetProjectGitRepositoryPullRequests(projectKey, repoName);
            Assert.True(actual.Count > 0);
            var pr = actual[0];
            Assert.Equal(2, pr.Id);
            Assert.Equal(3, pr.ProjectId);
            Assert.Equal(5, pr.RepositoryId);
            Assert.Equal(1, pr.Number);
            Assert.Equal("test", pr.Summary);
            Assert.Equal("test data", pr.Description);
        ...
        public override void AddProjectGitRepositoryPullRequestTest()
        {
            ...
            var repoId = backlog.GetProjectGitRepositories(projectId)[0].Id;
            var r = new Random();
            var summary = $"sum.{r.Next()}";
            var desc = $"desc.{r.Next()}";
            var option = new AddPullRequestOptions(summary, desc, "master", "develop")
            {
                AssigneeId = r.Next(),
                IssueId = r.Next()
            };
            var actual = backlog.AddProjectGitRepositoryPullRequest(projectId, repoId, option);
            Assert.Equal(summary, actual.Summary);
            Assert.Equal(desc, actual.Description);
            Assert.Equal(option.AssigneeId, actual.Assignee.Id);
            Assert.Equal(option.IssueId, actual.Issue.Id);
        }
        ...
        public override void AddProjectGitRepositoryPullRequestCommentTest()
        {
            ...
            var repoId = r.Next();
            var number = r.Next();
            var content = $"comment.{r.Next()}";
            var comment = new AddPullRequestCommentOptions(content);

            var actual = backlog.AddProjectGitRepositoryPullRequestComment(projectId, repoId, number, comment);
            Assert.Equal(content, actual.Content);
        }
        ...
        public override void UpdateProjectGitRepositoryPullRequestCommentTest()
        {
            ...
            var repoId = r.Next();
            var number = r.Next();
            var commentId = r.Next();
            var secondComment = $"second.{r.Next()}";
            var actual = backlog.UpdateProjectGitRepositoryPullRequestComment(projectId, repoId, number, commentId, secondComment);
        ...
        public override void DownloadProjectGitRepositoryPullRequestAttachmentTest()
        {
            ...
            var number = r.Next();
            var fileId = r.Next();
            var actual = backlog.GetProjectGitRepositoryPullRequestAttachment(projectId, repoId, number, fileId);
            var fileName = string.Format("{0}.{1}.dat", number, fileId);
